//go:generate mockgen -destination=mocks/mocks.go -source=providers.go
package internal
...
func (*HttpCleint) Get(string) []byte {
	return []byte("example")
}
...
func NewUserService(repo *UserRepo, httpClient HTTP) *UserService {
...
func NewHttpCleint(cfg *Config) *HttpCleint {
	println("init http client")
